...
public class B_11505 {
    ...
    public static long makeTree(int start, int end, long[] arr, long[] segmentTree ,int nodeNum){
        ...
        int mid = (start + end) / 2;
        return segmentTree[nodeNum] = (makeTree(start, mid, arr, segmentTree, nodeNum * 2) * makeTree(mid + 1, end, arr, segmentTree, nodeNum * 2 + 1)) % mod;
    }
    ...
    public static long modifyTree(int start, int end, long[] segmentTree, int nodeNum, int idx, long modifyNum){
        ...
        if(start <= idx && idx <= mid){
            return segmentTree[nodeNum] = (modifyTree(start, mid, segmentTree, nodeNum * 2, idx, modifyNum) * segmentTree[nodeNum * 2 + 1]) % mod;
        }
        else{
            return segmentTree[nodeNum] = (segmentTree[nodeNum * 2] * modifyTree(mid + 1, end, segmentTree, nodeNum * 2 + 1, idx, modifyNum)) % mod;
        }
    }
}
